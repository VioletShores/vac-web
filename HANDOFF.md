...
## S111 addendum 20 (11 Jun) — F-563 legibility: greeting first-class gate (5e60e1d) + live equaliser (3667b85). The "did it hear me?" gap.

### (1) GREETING = FIRST-CLASS GATE (5e60e1d)
The greeting was the last second-class part (a text block + timer, no feedback) and as the FIRST thing the user hits, its vagueness set the uncertain tone. renderGreeting() now gives it digit-parity: the ACTUAL greeting text BIG + yellow (read the exact words at a glance), a "READ THIS"→"🎙️ LISTENING" state, and a "✓ Heard it" beat (GREET_HEARD_BEAT_MS) before advancing — so the user KNOWS it registered. Pacing only — phraseSpoke gate unchanged. Codex (4 rounds): KEEP the PHRASE_DURATION timer floor (no early-advance — a mid-greeting pause can trip phraseSpoke on a PARTIAL greeting → server-side fail); voice-only keeps the full window + no "Heard it" (speaks greeting+numbers in phase 1); "✓ Heard it" shows neutral "One moment…" not "now the numbers" during the heard→floor gap.

### (2) LIVE EQUALISER (3667b85) — DISPLAY-ONLY
Root of the "is it hearing me?" uncertainty = no live mic-level proof. _renderEqualiser(rms) drives stable bars (#vacEqGreeting in the challenge panel, #vacSayEq in the say-cover) off the mic level startAudioMonitor already computes every frame — teal voiced / dim quiet. Shown during the greeting + each digit's say-step. Double duty: "we hear you" + mic-dead indicator (flat bars = no mic, complements the no-mic recovery). GUARDRAIL (codex-confirmed): reads the level + writes only bar height/colour — does NOT touch the voice gate (silence→onset binding) or advance. Eq lives OUTSIDE challengeText (which renderGreeting rewrites per tick) so it stays smooth; hidden on every phrase exit + finish/reset.
